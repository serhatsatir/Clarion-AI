   ...
   INCLUDE('NetWww.inc'),ONCE
                     ...
                     MAP
                       INCLUDE('AI002.INC'),ONCE        !Local module procedure declarations
                     END
...
Window               WINDOW('Gemini Example'),AT(,,386,224),FONT('Segoe UI',10,,FONT:regular),DOUBLE,CENTER,ICON('gemini.ico'), |
  GRAY,IMM,SYSTEM
                       ...
                       BUTTON('Post'),AT(328,49,50,12),USE(?GetPage),LEFT,ICON('gemini.ico'),TIP('Send your re' & |
  'quest to ChatGPT')
  ...
  'web page: https://ai.google.dev/gemini-api/docs/api-key')
...
ReformatQuestion       ROUTINE
    ...
    Loc:PostString='{{"contents": [{{"parts":[{{"text": "'&st.GetValue()&'Write a story about a magic backpack."}]}]}'
    !!!Loc:PostString='{{"prompt": [{{"parts":[{{"text": "'&st.GetValue()&'Write a story about a magic backpack."}]}]}'
    Loc:PostString=st.AnsiToUtf8(Loc:PostString,uz#,st:CP_ISO_8859_9)
    st.Trace(Loc:PostString)
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('AI_Gemini')
  ...
  MyQuestion = INIMgr.TryFetch('AI_Gemini_PreservedVars','MyQuestion')
  API_Key = INIMgr.TryFetch('AI_Gemini_PreservedVars','API_Key')
  API_temp = INIMgr.TryFetch('AI_Gemini_PreservedVars','API_temp')
  ...
  INIMgr.Fetch('AI_Gemini',Window)                         ! Restore window settings from non-volatile store
  SELF.SetAlerts()
  RETURN ReturnValue
...
ReturnValue          BYTE,AUTO
  ...
  IF ReturnValue THEN RETURN ReturnValue.
  IF SELF.Opened
    INIMgr.Update('AI_Gemini',Window)                      ! Save window data to non-volatile store
  END
  ! Save preserved local variables in non-volatile store
  INIMgr.Update('AI_Gemini_PreservedVars','MyQuestion',MyQuestion)
  INIMgr.Update('AI_Gemini_PreservedVars','API_Key',API_Key)
  INIMgr.Update('AI_Gemini_PreservedVars','API_temp',API_temp)
...
Looped BYTE
  ...
  ReturnValue = PARENT.TakeAccepted()
    ...
    OF ?GetPage
      ThisWindow.Update()
      IF API_Key='' THEN
          MESSAGE('Create your own API key using following web page: https://ai.google.dev/gemini-api/docs/api-key','Gemini API Key',ICON:Exclamation,,,MSGMODE:CANCOPY)
          CYCLE
      END
      ...
      ThisWebClient.Post('https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=AIzaSyCUsyqfMKJ_pANW4lCN09KTw7zQ9kuYRRI',Loc:PostString)
    END
    RETURN ReturnValue
  END
...
ThisWebClient.ErrorTrap PROCEDURE(string errorStr,string functionName)
  ...
  Message ('Cevap alýnamadý. Hata ' & ThisWebClient.Error & |
...
ThisWebClient.PageReceived PROCEDURE
  ...
  ELSIF SELF.ThisPage.Length() < size (Loc:GetString)
      ...
      Loc:Status='Sorunuzun cevabý alýndý.'
      DO ReformatAnswer
      ThisWebClient.Close()
  ELSE
      Loc:Status = '* Gelen cevap ' & SELF.ThisPage.Length() & |
               ' bayt, ekranda görebildiðiniz kýsým ' & size (Loc:GetString) & ' bayt.'
      Loc:GetString=SELF.ThisPage.GetValue()
      DO ReformatAnswer
      ThisWebClient.Close()
  END
