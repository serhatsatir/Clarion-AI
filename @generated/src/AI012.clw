   ...
   INCLUDE('NetWww.inc'),ONCE
                     ...
                     MAP
                       INCLUDE('AI012.INC'),ONCE        !Local module procedure declarations
                     END


!!! <summary>
!!! Generated from procedure template - Window
!!! You can chat with DeepSeek from this window
!!! </summary>
AI_DeepSeek PROCEDURE 
...
Window               WINDOW('ChatGPT Example'),AT(,,386,224),FONT('Segoe UI',10,,FONT:regular),DOUBLE,CENTER,ICON('chatgpt.ico'), |
  ...
  TRN
                       ...
                       SPIN(@n3.1),AT(73,64,,10),USE(API_temp),RIGHT(1),RANGE(0,1.5),STEP(0.1),TIP('Coding / M' & |
  'ath?0.0<0DH,0AH>Data Cleaning / Data Analysis<09H>1.0<0DH,0AH>General Conversation<09H>' & |
  '1.3<0DH,0AH>Translation<09H>1.3<0DH,0AH>Creative Writing / Poetry<09H>1.5')
                       STRING('API'),AT(7,49,,24),USE(?STRING1),FONT(,,,FONT:bold,CHARSET:DEFAULT),CENTER,ANGLE(900), |
  COLOR(00D3D3D3h)
                       PROMPT('It works differently from OpenAI / ChatGPT. Please check the tooltip.'),AT(106,65), |
  USE(?PROMPT1),FONT(,8,COLOR:Maroon,,CHARSET:DEFAULT)
                     END
...
ReformatTextCompletion      ROUTINE
    ...
    st.Prepend('{{"model": "deepseek-chat","messages": [{{"role": "user", "content": "')
...
GetTextMessage      ROUTINE
    ...
    if st.setBetween('"content": "', '},') = st:notFound
        if st.setBetween('"content": "', '"finish_reason":') = st:notFound
            Loc:Status='Couldn''t get legal response from DeepSeek'
            EXIT
        END
    END
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('AI_DeepSeek')
  ...
  MyQuestion = INIMgr.TryFetch('AI_DeepSeek_PreservedVars','MyQuestion')
  API_Key = INIMgr.TryFetch('AI_DeepSeek_PreservedVars','API_Key')
  API_temp = INIMgr.TryFetch('AI_DeepSeek_PreservedVars','API_temp')
  ...
  INIMgr.Fetch('AI_DeepSeek',Window)                       ! Restore window settings from non-volatile store
  SELF.SetAlerts()
  RETURN ReturnValue
...
ReturnValue          BYTE,AUTO
  ...
  IF ReturnValue THEN RETURN ReturnValue.
  IF SELF.Opened
    INIMgr.Update('AI_DeepSeek',Window)                    ! Save window data to non-volatile store
  END
  ! Save preserved local variables in non-volatile store
  INIMgr.Update('AI_DeepSeek_PreservedVars','MyQuestion',MyQuestion)
  INIMgr.Update('AI_DeepSeek_PreservedVars','API_Key',API_Key)
  INIMgr.Update('AI_DeepSeek_PreservedVars','API_temp',API_temp)
...
Looped BYTE
  ...
  ReturnValue = PARENT.TakeAccepted()
    ...
    OF ?GetPage
      ...
      Loc:GetString=''
      ChatGPTResponse=''
      Loc:URL='https://api.deepseek.com/v1/chat/completions'
